...
• [순풍] 흐름 제목 — 무엇이 어디서 어디로 움직였는지 2~3문장. 숫자 포함.
  ...
  근거 ④⑤
(판정이 아니다. 조치·판단을 쓰지 말 것. 최대 6건. 해당 없으면: 없음)

★ 흐름 항목 규칙 (위반 시 신호가 사라진다)
- 맨 앞에 방향을 [순풍]/[역풍]/[중립]/[불명] 으로 반드시 표기할 것.
  검색 결과의 direction 을 그대로 쓸 것. 임의로 바꾸지 말 것.
- ★ 방향이 다른 흐름을 한 항목으로 합치지 말 것. 역풍과 순풍을 "지연·표준화"
  같은 제목으로 묶으면 어느 쪽으로 움직였는지가 통째로 사라진다. 따로 쓸 것.
- 항목이 6건을 넘으면 억지로 합치지 말고 **중요도 낮은 것을 빼라.**
- 벤더·제품 고유명(예: Spectrum-X Photonics, Rubin Ultra/Kyber, HBM4)이 있으면
  반드시 남길 것. "CPO 확산" 같은 일반명으로 뭉개면 무슨 일이 났는지 알 수 없다.
- 이전에 논란·상충 보도가 있던 사안이 사실로 결착된 흐름은 우선해서 실을 것.
...
## ⚡ 오늘의 큰 건
판정이 아니다. 등급을 매기지 말 것. 위 '오늘의 큰 건' 재료만 쓸 것.
- **사건명** — 무슨 일인지 2~3문장. 확인된 사실만. 1차 확인이 안 됐으면 명시.
  포지션별 영향:
  | 포지션 | 방향·강도 | 전달 경로 | 시차 | 확인 지표 |
  |---|---|---|---|---|
  | 포지션명 (회사명 티커) | 순풍·직접 | A 증가 → B 경유 → C 변화 | 2~3분기 | 무엇을 보면 아는지 |
  근거: ①②
(strength='없음' 인 포지션은 표에서 뺄 것. 해당 없으면 "없음")

## 🔷 AI 인프라 흐름 (Layer 0.5)
판정이 아니다. 등급을 매기지 말 것. 테마별로.
검색 결과의 흐름을 **하나도 빼지 말고** 각각 별도 항목으로 쓸 것 (텔레그램만 6건 제한).
방향이 다른 흐름을 합치지 말 것.
- **[순풍] 흐름 제목** — [W2] 해당 watch_shift 원문
...
## 🌐 감시 목록 밖
위 '감시 목록 밖' 재료만 쓸 것. 1차 출처를 확인하지 않은 채널 원문이다.
근거 마커를 붙이지 말고, "~라고 전해진다" 로 쓸 것. 사실로 단정하지 말 것.
- **한 줄 요약** — 내 포지션과 어떻게 닿을 수 있는지 한 마디
  출처: 채널명 (미검증)
(해당 없으면 "없음")
...
★★ 신선도 (이걸 안 지키면 몇 주 전 뉴스가 오늘의 흐름으로 둔갑한다)
- finding 하나하나는 **최근 14일 안에 실제로 일어난 변화**여야 한다.
  그보다 오래된 사실은 배경일 뿐 오늘의 흐름이 아니다.
  실측 사례: 7월 말에 발표된 상반기 수주 실적이 9월 7일 다이제스트에
  "신규 관측" 으로 올라왔다. 이런 건 아예 담지 말 것.
- 출처가 전부 14일보다 오래됐으면 그 finding 을 만들지 말 것. 배경 설명이
  필요하면 최근 변화를 설명하는 문장 안에서만 쓸 것.
- 예외: 이전 관측 기록에 이미 있는 흐름을 **이번에 새 근거로 갱신**하는 경우.
  이때는 무엇이 새로 확인됐는지를 summary 앞머리에 쓸 것.
- 오래된 자료밖에 못 찾았으면 findings 를 비우고 observations 만 갱신할 것.

★ 단순 주가 등락은 흐름이 아니다.
  "주가가 며칠 새 얼마 빠졌다 / 촉발 요인은 미확인" 류를 finding 으로 만들지 말 것.
  시세 사이트를 출처로 삼는 항목은 담지 말 것. 가격은 다이제스트가 따로 표기한다.
  주가가 움직인 **원인이 되는 사실**(수주·규제·실적·공급망 변화)을 찾았을 때만 담는다.

이미 널리 알려진 배경 설명은 쓰지 말 것.
